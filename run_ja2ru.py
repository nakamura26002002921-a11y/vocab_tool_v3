...
「etymology」は漢字の成り立ち、日本語としての語源・由来などをロシア語で記載する。最大100ロシア語単語程度を目安とする。Web検索結果で確認できない情報は推測しない。漢字の字源と日本語の語源を混同しない。諸説ある場合は、Web検索で確認できた説だけを記載する。確認できない場合は「""」とする。
「collocations」はWeb検索で確認できた自然な日本語のコロケーションを最大3つ記載する。「日本語（ロシア語訳）」の形式とする。日本語の語や表現に漢字が含まれる場合は、通常の日本語としての自然な読みを「漢字を含む表記（ひらがなの読み）」の形式で付ける。漢字1文字ごとに機械的に読みを付けない。例えば「家を建てる」は「家（いえ）を建（た）てる」ではなく、必要に応じて自然な語単位で読みを付ける。「青二才」は「青二才（あおにさい）」、「青空」は「青空（あおぞら）」とする。確認できたものが3つ未満の場合は、確認できたものだけを記載する。存在しないコロケーションを推測して補完しない。確認できない場合は「[]」とする。
「examples」は必ず2つ作成する。「{word}」またはその自然な活用形を含む、実際の日本語として自然な例文を作成する。可能であれば異なる用法の例文を2つ作成する。不自然な例文や、対象単語と関係の薄い例文を作成しない。日本語の例文に漢字が含まれる場合は、通常の日本語として自然な読みを「漢字を含む表記（ひらがなの読み）」の形式で付ける。漢字1文字ごとに機械的に読みを付けない。例えば「仕事が終わったら家に帰ります。」は「仕事（しごと）が終（お）わったら家（いえ）に帰（かえ）ります。」とする。「青二才」は「青二才（あおにさい）」のように語単位で読みを付ける。
