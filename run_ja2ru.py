...
「meaning」はロシア語で代表的な意味を最大3つ記載し、複数は「;」で区切る。確認できない場合は「""」とする。「part_of_speech」は英語で記載し、確認できない場合は「""」とする。「etymology」は漢字の成り立ち、日本語としての語源・由来などを100文字以内で記載し、確認できない場合は「""」とする。
「collocations」は確認できたものを最大3つ、「日本語（ロシア語訳）」の形式で記載する。3つ未満なら確認できたものだけとし、存在しないものは補完しない。確認できない場合は「[]」とする。
「examples」は必ず2つ作成する。「{word}」または自然な活用形を含む自然な日本語とし、可能なら異なる用法を使う。「example_translated」は自然で正確なロシア語訳とする。
